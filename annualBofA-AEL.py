...
DFcommon = pandas.read_excel('annualBofA.xlsx', sheet_name = 'volatility')
vix = DFcommon.values[:, 1].astype(float)
N = len(vix)
allratings = ['AAA', 'AA', 'A', 'BBB']
allYears = range(1996, 2026)
for ratings in allratings:
    plt.plot(allYears, DFrates[ratings], label = ratings)
plt.legend()
plt.xlabel('Year')
...
# for ratings in allratings:
#     plt.plot(allYears, DFwealth[ratings]/DFwealth[ratings].iloc[0], label = ratings)
# plt.legend()
# plt.title('Wealth process for corporate rated bonds, 1996 = 1$')
# plt.xlabel('Year')
# plt.show()

plt.plot(allYears, vix)
plt.title('Annual VIX')
plt.show()
...
for ratings in allratings:
    ...
    wealth = DFwealth[ratings].values
    
    # Relabeling the data, picking the series for this ratings
    tr = numpy.diff(wealth)/wealth[:-1]
    rate = DFrates[ratings].values
